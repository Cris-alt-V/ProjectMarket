...
<?php $__env->startSection('content'); ?>
  <div class="product-detail-container">
    <div class="product-detail-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 40px; padding: 40px;">
      <div>
        <img src="<?php echo e($producto->imagen_url ? (\Illuminate\Support\Str::startsWith($producto->imagen_url, ['http://','https://','//']) ? $producto->imagen_url : asset(ltrim($producto->imagen_url, '/'))) : ''); ?>" alt="<?php echo e($producto->nombre); ?>" id="productImage" class="product-image-large">
      ...
      <div class="product-details">
        ...
        <?php if(session('user') && session('user')['id_usuario'] !== $producto->id_vendedor): ?>
          <div class="seller-contact-card">
            ...
            <a
              ...
              href="<?php echo e(url('/mensajes')); ?>?product_id=<?php echo e($producto->id_producto); ?>&receiver_id=<?php echo e($producto->id_vendedor); ?>&product_name=<?php echo e(urlencode($producto->nombre)); ?>&partner_name=<?php echo e(urlencode($comercio->nombre_negocio)); ?>"
            >
        ...
        <div class="actions" style="display: flex; gap: 15px; margin-bottom: 30px; flex-wrap: wrap;">
          <button class="btn btn-primary" id="btnAddToCart"><?php echo e(session('user') && session('user')['id_usuario'] === $producto->id_vendedor ? 'No puedes comprar tu producto' : 'Agregar al Carrito'); ?></button>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    const product = {
      ...
      foto: `<?php echo e($producto->imagen_url ?? ''); ?>`,
    ...
    const productSellerId = <?php echo e($producto->id_vendedor); ?>;

    const btnAdd = document.getElementById('btnAddToCart');
    const currentUser = window.currentSessionUser || (window.MarketplaceApp && MarketplaceApp.getCurrentUser ? MarketplaceApp.getCurrentUser() : null);
    const isOwnProduct = currentUser && Number(currentUser.id_usuario) === Number(product.comercioId);
    if (btnAdd) {
      if (isOwnProduct) {
        btnAdd.disabled = true;
        btnAdd.textContent = 'No puedes comprar tu producto';
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\crist\OneDrive\Desktop\casdq\ProjectMarket\market\resources\views/detalle-producto.blade.php ENDPATH**/ ?>