...
@section('content')
  <div class="product-detail-container">
    <div class="product-detail-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 40px; padding: 40px;">
      <div>
        <img src="{{ $producto->imagen_url ? (\Illuminate\Support\Str::startsWith($producto->imagen_url, ['http://','https://','//']) ? $producto->imagen_url : asset(ltrim($producto->imagen_url, '/'))) : '' }}" alt="{{ $producto->nombre }}" id="productImage" class="product-image-large">
      ...
      <div class="product-details">
        ...
        <div class="product-meta" style="display: flex; flex-wrap: wrap; gap: 20px; margin-bottom: 20px; border-bottom: 1px solid #eee; padding-bottom: 20px;">
          ...
          <div class="meta-item"><strong>Categoría:</strong> <span id="productCategory">{{ $producto->categoria ?? 'General' }}</span></div>
          <div class="meta-item"><strong>Existencias:</strong> <span id="productStock">{{ $producto->stock > 0 ? $producto->stock : 'sin existencias' }}</span></div>
        ...
        <div class="actions" style="display: flex; gap: 15px; margin-bottom: 30px; flex-wrap: wrap;">
          <button class="btn btn-primary" id="btnAddToCart">{{ session('user') && session('user')['id_usuario'] === $producto->id_vendedor ? 'No puedes comprar tu producto' : 'Agregar al Carrito' }}</button>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    const product = {
      ...
      categoria: `{{ addslashes($producto->categoria ?? 'General') }}`,
      ubicacion: `{{ addslashes($comercio->ubicacion) }}`,
      comercioId: {{ $comercio->id_vendedor }},
      stock: parseInt({{ $producto->stock ?? 0 }}, 10) || 0,
    };
    const productSellerId = {{ $producto->id_vendedor }};
